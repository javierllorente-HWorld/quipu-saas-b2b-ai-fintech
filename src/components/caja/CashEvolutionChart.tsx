"use client";

import * as React from "react";
import type { CurrencyCode } from "@/components/inicio/mock";
import { formatMoney, formatShortDate } from "@/components/inicio/format";
import type { CashEvolutionPoint } from "./mock";

export type CashEvolutionChartProps = {
  points: CashEvolutionPoint[];
  currency: CurrencyCode;
};

function extent(values: number[]) {
  const min = Math.min(...values);
  const max = Math.max(...values);
  return { min, max };
}

function scaleY(value: number, min: number, max: number, height: number) {
  if (max === min) return height / 2;
  const t = (value - min) / (max - min);
  return height - t * height;
}

function linePath(
  values: number[],
  min: number,
  max: number,
  w: number,
  h: number,
  padX: number,
) {
  const usableW = w - padX * 2;
  const step = values.length <= 1 ? usableW : usableW / (values.length - 1);
  return values
    .map((v, i) => {
      const x = padX + i * step;
      const y = scaleY(v, min, max, h);
      return `${i === 0 ? "M" : "L"}${x.toFixed(2)},${y.toFixed(2)}`;
    })
    .join(" ");
}

export function CashEvolutionChart({ points, currency }: CashEvolutionChartProps) {
  const w = 820;
  const h = 220;
  const padX = 28;

  const actual = points.map((p) => p.actual);
  const projected = points.map((p) => p.projected);

  const { min, max } = extent([...actual, ...projected]);
  const pad = (max - min) * 0.06;
  const minY = Math.min(min - pad, 0);
  const maxY = max + pad;

  const actualPath = linePath(actual, minY, maxY, w, h, padX);
  const projectedPath = linePath(projected, minY, maxY, w, h, padX);

  const last = points.at(-1);
  const lastProjected = last?.projected ?? 0;

  return (
    <div className="qp-card">
      <div className="qp-card-header">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-base font-semibold tracking-tight">
              Evolución diaria de caja
            </div>
            <div className="mt-1 text-sm text-muted-foreground">
              Saldo real vs proyección.
            </div>
          </div>

          <div className="flex items-center gap-2">
            <Segment label="30 días" active />
            <Segment label="90 días" />
            <Segment label="12 meses" />
            <Segment label="Diario" />
          </div>
        </div>
      </div>

      <div className="qp-card-content">
        <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-start">
          <div className="w-full overflow-hidden">
            <svg
              viewBox={`0 0 ${w} ${h}`}
              className="h-[220px] w-full"
              role="img"
              aria-label="Gráfico de evolución de caja"
            >
              <defs>
                <linearGradient id="quipuActualFill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0" stopColor="rgba(15,76,255,0.16)" />
                  <stop offset="1" stopColor="rgba(15,76,255,0)" />
                </linearGradient>
              </defs>

              <path
                d={`M${padX},${scaleY(actual[0] ?? 0, minY, maxY, h).toFixed(2)} ${actualPath.slice(1)}`}
                stroke="none"
                fill="none"
              />

              <path
                d={`${actualPath} L${w - padX},${h} L${padX},${h} Z`}
                fill="url(#quipuActualFill)"
                stroke="none"
              />

              <path
                d={actualPath}
                stroke="rgba(15,76,255,1)"
                strokeWidth="2.6"
                fill="none"
              />

              <path
                d={projectedPath}
                stroke="rgba(7,27,74,0.55)"
                strokeWidth="2.2"
                fill="none"
                strokeDasharray="6 6"
              />

              {points.map((p, i) => {
                const usableW = w - padX * 2;
                const step =
                  points.length <= 1 ? usableW : usableW / (points.length - 1);
                const x = padX + i * step;
                const y = scaleY(p.actual, minY, maxY, h);
                return (
                  <g key={p.date}>
                    <circle cx={x} cy={y} r="3.3" fill="rgba(15,76,255,1)" />
                  </g>
                );
              })}
            </svg>

            <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              <LegendDot color="bg-[color:var(--quipu-accent)]" label="Saldo real" />
              <LegendDot
                color="bg-[color:var(--quipu-night)]"
                label="Proyección"
                dashed
              />
            </div>

            <div className="mt-3 flex justify-between text-xs text-muted-foreground">
              <span>{points[0]?.date ? formatShortDate(points[0].date) : ""}</span>
              <span>
                {points.at(-1)?.date ? formatShortDate(points.at(-1)!.date) : ""}
              </span>
            </div>
          </div>

          <div className="hidden rounded-2xl border border-border bg-white/60 px-4 py-3 text-right lg:block">
            <div className="text-xs text-muted-foreground">Proyección (7 días)</div>
            <div className="mt-1 text-lg font-semibold text-foreground">
              {formatMoney(lastProjected, currency)}
            </div>
            <div className="mt-2 text-xs text-muted-foreground">
              Basado en tendencia reciente.
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Segment({ label, active }: { label: string; active?: boolean }) {
  return (
    <button
      type="button"
      className={[
        "h-9 rounded-full px-3 text-xs font-semibold transition",
        active
          ? "bg-[color:var(--quipu-ice)] text-[color:var(--quipu-night)] ring-1 ring-[color:var(--quipu-border)]"
          : "border border-border bg-card text-muted-foreground hover:bg-white/70",
      ].join(" ")}
      aria-pressed={active ? "true" : "false"}
    >
      {label}
    </button>
  );
}

function LegendDot({
  color,
  label,
  dashed,
}: {
  color: string;
  label: string;
  dashed?: boolean;
}) {
  return (
    <span className="inline-flex items-center gap-2">
      <span
        className={[
          "inline-flex h-2.5 w-7 items-center justify-center rounded-full",
          dashed ? "bg-transparent" : color,
          dashed ? "ring-1 ring-black/15" : "",
        ].join(" ")}
      >
        {dashed ? <span className={`h-1 w-full ${color} opacity-70`} /> : null}
      </span>
      <span>{label}</span>
    </span>
  );
}

